...
def _build_cci_tooltip(insert_time: datetime.datetime | None):
    # 与你原 JS 完全同逻辑：≤1天 绿，否则粉；无数据 灰
    if not insert_time:
        return {
            "tip_text": "暂无 CCI 数据",
...
def get_latest_cci():
    try:
        # 前端传目标热值（不传则默认 5500）
        target_calorific = float(request.args.get("target_calorific", 5500))
        ...
        base_price = float(row["priceavg"])   # 5500 基准价
        cur = row["curdate"]
        ...
        tip = _build_cci_tooltip(insert_time)
...
def electric_blend():
    ...
    try:
        ...
        cci = {"success": False, "cci_price_text": "—", "tooltip": _build_cci_tooltip(None)}
        try:
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("""
