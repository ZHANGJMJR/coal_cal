...
async function loadLatestCCI() {
    try {
        ...
        async function loadLatestCCI(targetCalorific = 5500) {
            try {
                const res = await fetch(`/api/cci/latest?target_calorific=${targetCalorific}`);
                ...
                latestCCI = {
                    price: data.cci_price,                // ✅ 已折算
                    basePrice: data.cci_base_price,       // 5500 原始价
                    targetCalorific: data.target_calorific,
                    insertTime: new Date(data.insert_time),
                    tooltip: data.tooltip,
                    priceText: data.cci_price_text
                };
                return true;
            } catch (e) {
        ...
        // const data = await res.json();
        // // 后端会返回 tooltip，即使没有数据也会有默认灰色 tooltip
        // latestCCI = data;
        // return true;
    } catch (e) {
        console.error("获取 CCI 失败", e);
                ...
                <div class="flex items-center justify-end space-x-6 overflow-visible">
                    <div class="text-gray-800 text-base font-bold leading-8 text-right mr-8">
                        ...
                        <div class="flex justify-end whitespace-nowrap">
                            ...
                            <span class="line-through" style="font-size: 18px;">${plan.reverse_ceke_price ?? 400}元/吨</span>
                        </div>
                        ...
                        <div class="flex justify-end whitespace-nowrap">
                            <span>预测销售毛利</span>
                            <span style="display:inline-block; width:16px; text-align:center;">：</span>
                            <span style="font-size: 18px;" class="line-through text-blue-600">${plan.gross_profit ?? 100}元/吨</span>
                        </div>
                    </div>
